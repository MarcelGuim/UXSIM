...
get_important_edge_time_average = False
get_UXSIM_gif = False
#endregion
...
if get_edges_and_junctions_parquet:
    ...
    for e in edges_full.values():
        try:    
            ...
            k = 2
    edg_full.to_parquet(zone+"/edges_"+zone+"_important.parquet", index=False)
    junct_full.to_parquet(zone+"/junctions_"+zone+"_important.parquet", index=False)
...
def run_on_simulation(points_add, traffic_add):
    ...
    traffic = 0.01+ traffic_add*0.002
    time_traffic = max_time - 1800
    points = 250 + points_add*50
    ...
    flush_edge_data_to_disk(
        zone+"/RESULTS/simulation"+str(sim_num)+"_"+str(traffic_add)+"_"+str(points_add)+".parquet"
    )
    ...
    if get_important_edge_time_average:
        divide_data_in_time_slots("eixample/edges_eixample_important.parquet","eixample/RESULTS/simulation"+sim_num+".parquet", 30)
...
for points_add, traffic_add in vector:
    run_on_simulation(points_add, traffic_add)
